...
public class Rectangle extends Shape {
    ...
    // Boolean to track whether the rectangle is solid (filled) or not
    ...
    // Parameterized constructor to initialize the rectangle's position, color, and solid state
    public Rectangle(int p1, int p2, int p3, int p4, int c, boolean solid) {
    ...
    // Setter method to update the solid state of the rectangle
    void setSolid(boolean state) {
        solidState = state;
    }

    // Getter method to retrieve the solid state of the rectangle
    boolean getSolid() {
        return solidState;
    }
    ...
    // Method to draw the rectangle shape on the screen
    public void drawShape(Graphics g) {
        ...
        // Draw the rectangle based on the solid state
        if (solidState) {
            ...
            g.drawRect(x, y, width, height); // Draw an outlined rectangle
        }
    }
		
	// Method to draw a dotted rectangle
	private void drawDottedRectangle(Graphics g, int x, int y, int width, int height) {
		...
		// Draw the rectangle with the dashed stroke
